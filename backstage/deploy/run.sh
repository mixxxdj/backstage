...
if [ $1 = 'start' ];then
    ...
    sudo cp backstage_server /etc/nginx/sites-available/backstage_server
    sudo ln -s /etc/nginx/sites-available/backstage_server /etc/nginx/sites-enabled/backstage_server
    sudo cp backstage.ini /etc/uwsgi/apps-available/
    sudo chmod 777 /var/run/nginx.pid
    echo "*_* Deploy and copy scipts *_*"

elif [ $1 = 'update' ];then
    echo "update master branch source code and update static files"
    cd $(cd "$(dirname "$0")"; pwd)/../
